...
Статус: **alpha** (C++17 / Pico SDK). Основной функционал live-игры реализован;
паттерн-секвенсор и полная ADSR/timing-цепочка — сознательно отложены.
...
| Timing-эффекты (swing/humanize/quantize/legato) | ⚠ структуры+меню, в вывод не применяются | `TimingCfg` редактируется, конвейер времени не подключён |
| Gate/ADSR (attack/release) | ⚠ структуры+меню, в вывод не применяются | `GateCfg` редактируется |
| Voicing Strum/Roll в live-выводе | ⚠ хранится; играем Block | детерминированный timing — будущее |
...
3. Применение timing-эффектов (swing/humanize/quantize/legato) к длительностям ноты.
4. Gate/ADSR как модель тайминга (attack/release) с сегментами sync (ms/квант).
...
9. Сохранение паттернов на flash (сериализация struct-модели в последний сектор).
