...
| Запись в паттерн (realtime overdub, step-edit) | 🔶 частично | запись + базовый step-edit в EDIT готовы; в Rec+Play длительность ноты определяется временем удержания (сколько держишь = столько шагов длится, протяжка через tie), а удержание Rest вставляет паузу той же длины (силовая отсечка); overdub-наложение поверх звучащих шагов и полифония — дальше |
| Паттерн-инструмент **Транспонирование** (Shift+F, key5) | ✅ реализовано | пакетный сдвиг питча с превью и тональностной привязкой: нижняя строка OFFSET/KEY/SCALE/MODE (клик циклит поле, ↑/↓ правка, Rest — отмена, Shift+F — применить одной undoable-операцией); область = SELECT-диапазон или весь паттерн; MODE=Skip помечает к удалению (см. `05-input-mapping.md`) |
