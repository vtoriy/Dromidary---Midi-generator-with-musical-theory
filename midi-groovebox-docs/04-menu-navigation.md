...
1. **QUICK** — панель быстрых параметров (Level 1): строки-группы
   (Key/CHD/Arp/Time/ADR/Dens/Shape) с ячейками. Отдельных ячеек `PRM`
...
| **RND** (всегда) | Note **Range** (мин-макс нота, напр. `C1-C4`) | полоса span под строкой; клик по имени → полный Randomize DETAIL |
...
сам является отдельной целью навигации: с левой колонки первой строки (name-фокус)
наклон **вверх** переводит фокус на заголовок (режим инвертируется), клик
переключает `KB`↔`RND`, наклон **вниз** возвращает на строки. При смене режима
QUICK перестраивается (исчезают/появляются строки).
...
Та же логика у QUICK-строки **RND**: клик по ячейке входит в edit-mode, наклон
...
**Скринсейвер:** после заданного бездействия (System → **Idle Anim**,
10 000–120 000 мс, шаг 5000; минимум 10 с) без какого-либо ввода
(носи/джойстик/кнопки) Quick/Full автоматически переходит в Animation
(`AppLoop::update_idle_screensaver`). **Любое** касание тут же возвращает на тот
экран, где пользователь был до этого. Ручной Animation (через long-press) не
перезаписывается.